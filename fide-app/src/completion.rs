...
use fide_rpc::{plugin::PluginId, proxy::ProxyRpcHandler};
use lsp_types::{
...
use crate::{config::FIDEConfig, editor::EditorData, snippet::Snippet};
...
impl CompletionData {
    pub fn new(cx: Scope, config: ReadSignal<Arc<FIDEConfig>>) -> Self {
...
fn completion_lens_text(
    ...
    // Get only the first line of text, because FIDE does not currently support
