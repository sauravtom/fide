...
use crate::{
    command::{CommandKind, FIDECommand},
...
impl Doc {
    ...
    pub fn set_language(&self, language: FIDELanguage) {
    ...
    fn phantom_text(
        ...
        let hints = config
            ...
            .map(|(interval, inlay_hint)| {
                ...
                PhantomText {
                    ...
                    text,
                    affinity,
                    fg: Some(config.color(FIDEColor::INLAY_HINT_FOREGROUND)),
                    // font_family: Some(config.editor.inlay_hint_font_family()),
                    font_size: Some(config.editor.inlay_hint_font_size()),
                    bg: Some(config.color(FIDEColor::INLAY_HINT_BACKGROUND)),
                    under_line: None,
                }
            });
                            ...
                            .filter_map(|(iv, diag)| {
                                    ...
                                    && diag.severity < Some(DiagnosticSeverity::HINT)
                                {
                                    let fg = {
                                        ...
                                        {
                                            FIDEColor::ERROR_LENS_ERROR_FOREGROUND
                                        } else if severity
                                            == DiagnosticSeverity::WARNING
                                        {
                                            FIDEColor::ERROR_LENS_WARNING_FOREGROUND
                                        } else {
                                            // information + hint (if we keep that) + things without a severity
                                            FIDEColor::ERROR_LENS_OTHER_FOREGROUND
                                        };
        ...
        let completion_text = config
            ...
            .map(|completion| PhantomText {
                ...
                fg: Some(config.color(FIDEColor::COMPLETION_LENS_FOREGROUND)),
        ...
        if let Some(preedit) = self
            .preedit_phantom(Some(config.color(FIDEColor::EDITOR_FOREGROUND)), line)
        {
            text.push(preedit)
        }
    ...
    fn apply_layout_styles(
                        ...
                        && diag.severity < Some(DiagnosticSeverity::HINT)
                    {
                        ...
                        let color_name = match diag.severity {
                            Some(DiagnosticSeverity::ERROR) => {
                                FIDEColor::LAPCE_ERROR
                            }
                            _ => FIDEColor::LAPCE_WARN,
                        };
        ...
        if let Some(max_severity) = max_severity {
            let theme_prop = if max_severity == DiagnosticSeverity::ERROR {
                FIDEColor::ERROR_LENS_ERROR_BACKGROUND
            } else if max_severity == DiagnosticSeverity::WARNING {
                FIDEColor::ERROR_LENS_WARNING_BACKGROUND
            } else {
                FIDEColor::ERROR_LENS_OTHER_BACKGROUND
            };
