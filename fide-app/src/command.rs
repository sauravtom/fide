...
pub enum CommandKind {
    Workbench(FIDEWorkbenchCommand),
    Edit(EditCommand),
    Move(MoveCommand),
...
pub fn fide_internal_commands() -> IndexMap<String, FIDECommand> {
    let mut commands = IndexMap::new();

    for c in FIDEWorkbenchCommand::iter() {
        let command = FIDECommand {
...
pub enum FIDEWorkbenchCommand {
    ...
    #[cfg(target_os = "macos")]
    #[strum(message = "Install FIDE to PATH")]
    #[strum(serialize = "install_to_path")]
    InstallToPATH,

    #[cfg(target_os = "macos")]
    #[strum(message = "Uninstall FIDE from PATH")]
