...
use fide_core::{
    buffer::rope_text::RopeText, command::FocusCommand, language::FIDELanguage,
    line_ending::LineEnding, mode::Mode, movement::Movement, selection::Selection,
    syntax::Syntax,
};
use fide_rpc::proxy::ProxyResponse;
...
use crate::{
    command::{
        CommandExecuted, CommandKind, InternalCommand, FIDECommand, WindowCommand,
    },
    db::FIDEDb,
    ...
    workspace::{FIDEWorkspace, FIDEWorkspaceType, SshHost},
};
...
impl PaletteData {
    ...
    fn get_ssh_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let FIDEWorkspaceType::RemoteSSH(host) = &workspace.kind {
    ...
    fn get_wsl_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let FIDEWorkspaceType::RemoteWSL(host) = &workspace.kind {
    ...
    fn set_run_configs(&self, content: String) {
        ...
        if configs.is_none() {
            if let Some(path) = self.workspace.path.as_ref() {
                let path = path.join(".fide").join("run.toml");
                self.common
                    .internal_command
    ...
    fn get_run_configs(&self) {
        if let Some(workspace) = self.common.workspace.path.as_deref() {
            let run_toml = workspace.join(".fide").join("run.toml");
    ...
    fn get_languages(&self) {
        let langs = FIDELanguage::languages();
        let items = langs
            .iter()
        ...
        if let Some(item) = items.get(index) {
            match &item.content {
                PaletteItemContent::PaletteHelp { cmd } => {
                    ...
                    self.common.fide_command.send(cmd);
                }
                ...
                PaletteItemContent::Command { cmd } => {
                    self.common.fide_command.send(cmd.clone());
                }
                    ...
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: FIDEWorkspace {
                                kind: FIDEWorkspaceType::RemoteSSH(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::WslHost { host } => {
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: FIDEWorkspace {
                                kind: FIDEWorkspaceType::RemoteWSL(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::Language { name } => {
                    ...
                    if name.is_empty() || name.to_lowercase().eq("plain text") {
                        ...
                        let lang = match FIDELanguage::from_name(name) {
                            Some(v) => v,
                            None => return,
                        };
                ...
                PaletteItemContent::SCMReference { name } => {
                    self.common
                        .fide_command
                        .send(crate::command::FIDECommand {
                        kind: CommandKind::Workbench(
                            crate::command::FIDEWorkbenchCommand::CheckoutReference,
                        ),
            ...
            self.common.window_common.window_command.send(
                WindowCommand::SetWorkspace {
                    workspace: FIDEWorkspace {
                        kind: FIDEWorkspaceType::RemoteSSH(ssh),
                        path: None,
                        last_open: 0,
                    },
                },
