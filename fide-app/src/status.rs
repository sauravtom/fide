...
use fide_core::mode::{Mode, VisualMode};
use lsp_types::{DiagnosticSeverity, ProgressToken};

use crate::{
    app::clickable_icon,
    command::FIDEWorkbenchCommand,
...
pub fn status(
    ...
    _config: ReadSignal<Arc<FIDEConfig>>,
) -> impl View {
        ...
        stack((
            ...
            .style(move |s| {
                ...
                let (bg, fg) = match mode.get() {
                    Mode::Normal => (
                        FIDEColor::STATUS_MODAL_NORMAL_BACKGROUND,
                        FIDEColor::STATUS_MODAL_NORMAL_FOREGROUND,
                    ),
                    Mode::Insert => (
                        FIDEColor::STATUS_MODAL_INSERT_BACKGROUND,
                        FIDEColor::STATUS_MODAL_INSERT_FOREGROUND,
                    ),
                    Mode::Visual(_) => (
                        FIDEColor::STATUS_MODAL_VISUAL_BACKGROUND,
                        FIDEColor::STATUS_MODAL_VISUAL_FOREGROUND,
                    ),
                    Mode::Terminal => (
                        FIDEColor::STATUS_MODAL_TERMINAL_BACKGROUND,
                        FIDEColor::STATUS_MODAL_TERMINAL_FOREGROUND,
                    ),
                };
            ...
            stack((
                svg(move || config.get().ui_svg(FIDEIcons::SCM)).style(move |s| {
                    let config = config.get();
                ...
                label(branch).style(move |s| {
                    s.margin_left(10.0)
                        .color(config.get().color(FIDEColor::STATUS_FOREGROUND))
                        .selectable(false)
                }),
            ))
            .style(move |s| {
                ...
                .hover(|s| {
                    s.cursor(CursorStyle::Pointer).background(
                        config.get().color(FIDEColor::PANEL_HOVERED_BACKGROUND),
                    )
                })
            })
            ...
            .on_event(
                ...
                move |_| {
                    if pointer_down.get() {
                        workbench_command
                            .send(FIDEWorkbenchCommand::PaletteSCMReferences);
                    }
            ...
            ),
            {
                ...
                stack((
                    svg(move || config.get().ui_svg(FIDEIcons::ERROR)).style(
                        move |s| {
                            let config = config.get();
                        ...
                        move |s| {
                            s.margin_left(5.0)
                                .color(
                                    config
                                        .get()
                                        .color(FIDEColor::STATUS_FOREGROUND),
                                )
                                .selectable(false)
                        },
                    ),
                    svg(move || config.get().ui_svg(FIDEIcons::WARNING)).style(
                        move |s| {
                            let config = config.get();
                            let size = config.ui.icon_size() as f32;
                            s.size(size, size)
        ...
        stack((
            {
                ...
                let icon = {
                    ...
                    move || {
                        ...
                        {
                            FIDEIcons::SIDEBAR_LEFT
                        } else {
                            FIDEIcons::SIDEBAR_LEFT_OFF
                        }
                    }
                };
                clickable_icon(
                ...
                let icon = {
                    ...
                    move || {
                        if panel.is_container_shown(
                            ...
                            FIDEIcons::LAYOUT_PANEL
                        } else {
                            FIDEIcons::LAYOUT_PANEL_OFF
                        }
                    }
                };
                clickable_icon(
            ...
            },
            {
                ...
                let icon = {
                    ...
                    move || {
                        ...
                        {
                            FIDEIcons::SIDEBAR_RIGHT
                        } else {
                            FIDEIcons::SIDEBAR_RIGHT_OFF
                        }
                    }
                };
                clickable_icon(
    ...
    .style(move |s| {
        ...
        s.border_top(1.0)
            ...
            .background(config.color(FIDEColor::STATUS_BACKGROUND))
...
fn status_text<S: std::fmt::Display + 'static>(
    ...
    label(text).style(move |s| {
        ...
        s.display(display)
            ...
            .padding_horiz(10.0)
            .items_center()
            .color(config.color(FIDEColor::STATUS_FOREGROUND))
            .hover(|s| {
