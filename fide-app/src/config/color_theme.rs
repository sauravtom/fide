...
mod tests {
    ...
    use crate::{config::FIDEConfig, workspace::FIDEWorkspace};

    #[test]
    fn test_resolve() {
        // Mimicking load
        let workspace = FIDEWorkspace::default();

        let config = FIDEConfig::merge_config(&workspace, None, None);
        let mut fide_config: FIDEConfig = config.try_deserialize().unwrap();
...
"##;
        ...
        fide_config.available_color_themes =
            [("test".to_string(), ("test".to_string(), test_theme_cfg))]
                .into_iter()
                .collect();
        // fide_config.available_icon_themes = Some(vec![]);
        fide_config.core.color_theme = "test".to_string();
