...
use crate::{
    ...
    command::{
        CommandExecuted, CommandKind, InternalCommand, FIDECommand,
        FIDEWorkbenchCommand, WindowCommand,
    },
    completion::{CompletionData, CompletionStatus},
    config::FIDEConfig,
    db::FIDEDb,
    debug::{DapData, FIDEBreakpoint, RunDebugMode, RunDebugProcess},
    doc::DocContent,
    ...
    workspace::{FIDEWorkspace, FIDEWorkspaceType, WorkspaceInfo},
};
...
pub struct CommonData {
    pub workspace: Arc<FIDEWorkspace>,
    pub scope: Scope,
    ...
    pub fide_command: Listener<FIDECommand>,
    pub workbench_command: Listener<FIDEWorkbenchCommand>,
    ...
    pub config: ReadSignal<Arc<FIDEConfig>>,
    pub proxy_status: RwSignal<Option<ProxyStatus>>,
    pub mouse_hover_timer: RwSignal<TimerToken>,
    pub breakpoints: RwSignal<BTreeMap<PathBuf, BTreeMap<usize, FIDEBreakpoint>>>,
...
pub struct WindowTabData {
    ...
    pub proxy: ProxyData,
    pub set_config: WriteSignal<Arc<FIDEConfig>>,
...
impl WindowTabData {
    ...
    pub fn new(
        ...
        let config = FIDEConfig::load(
            &workspace,
            &all_disabled_volts,
            &window_common.extra_plugin_paths,
        );
        let fide_command = Listener::new_empty(cx);
                    ...
                    .map(|(path, breakpoints)| {
                        (
                            ...
                            breakpoints
                                .into_iter()
                                .map(|b| (b.line, b))
                                .collect::<BTreeMap<usize, FIDEBreakpoint>>(),
                        )
                    })
                    .collect(),
            );
...

        {
            ...
            window_tab_data.common.fide_command.listen(move |cmd| {
                window_tab_data.run_fide_command(cmd);
            });
        }
    ...
    pub fn run_fide_command(&self, cmd: FIDECommand) {
        match cmd.kind {
        ...
        match cmd {
            ...
            InstallToPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        ...
                        arguments: vec![String::from("-e"), format!(r#"do shell script "ln -sf '{}' /usr/local/bin/fide" with administrator privileges"#, std::env::args().next().unwrap())],
                    }
                )
            }
            #[cfg(target_os = "macos")]
            UninstallFromPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        program: String::from("osascript"),
                        arguments: vec![String::from("-e"), String::from(r#"do shell script "rm /usr/local/bin/fide" with administrator privileges"#)],
                    }
                )
            }
            JumpLocationForward => {
