...
fn get_github_api(url: &str) -> Result<String> {
    let user_agent = format!("FIDE/{}", fide_core::meta::VERSION);
    let resp = fide_proxy::get_url(url, Some(user_agent.as_str()))?;
...
pub fn find_grammar_release() -> Result<ReleaseInfo> {
    let releases: Vec<ReleaseInfo> = serde_json::from_str(&get_github_api(
        "https://api.github.com/repos/fide/tree-sitter-grammars/releases?per_page=100",
    ).context("Failed to retrieve releases for tree-sitter-grammars")?)?;

    use fide_core::meta::{RELEASE, ReleaseType, VERSION};
