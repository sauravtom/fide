...
use fide_core::directory::Directory;
use fide_proxy::plugin::wasi::find_all_volts;
use fide_rpc::plugin::VoltID;
...
use crate::workspace::{FIDEWorkspace, FIDEWorkspaceType};
...
static DEFAULT_CONFIG: Lazy<config::Config> = Lazy::new(FIDEConfig::default_config);
static DEFAULT_LAPCE_CONFIG: Lazy<FIDEConfig> =
    Lazy::new(FIDEConfig::default_fide_config);
...
static DEFAULT_DARK_THEME_COLOR_CONFIG: Lazy<ColorThemeConfig> = Lazy::new(|| {
    let (_, theme) =
        FIDEConfig::load_color_theme_from_str(DEFAULT_DARK_THEME).unwrap();
...
impl FIDEConfig {
    pub fn load(
        ...
        let mut fide_config: FIDEConfig = match config.try_deserialize() {
            Ok(config) => config,
            Err(error) => {
        ...
        fide_config.available_color_themes =
            Self::load_color_themes(disabled_volts, extra_plugin_paths);
        fide_config.available_icon_themes =
            Self::load_icon_themes(disabled_volts, extra_plugin_paths);
        fide_config.resolve_theme(workspace);

        fide_config.color_theme_list = fide_config
            .available_color_themes
            .values()
            .map(|(name, _)| name.clone())
            .sorted()
            .collect();
        fide_config.color_theme_list.sort();

        fide_config.icon_theme_list = fide_config
            .available_icon_themes
            .values()
            .map(|(name, _, _)| name.clone())
            .sorted()
            .collect();
        fide_config.icon_theme_list.sort();

        fide_config.wrap_style_list = im::vector![
        ...
        fide_config.terminal.get_indexed_colors();
        ...
        match workspace.kind {
            FIDEWorkspaceType::Local => {
                if let Some(path) = workspace.path.as_ref() {
                    let path = path.join("./.fide/settings.toml");
                    config = config::Config::builder()
                        .add_source(config.clone())
                        .add_source(
            ...
            FIDEWorkspaceType::RemoteSSH(_) => {}
            #[cfg(windows)]
            FIDEWorkspaceType::RemoteWSL(_) => {}
        }
    ...
    fn default_fide_config() -> FIDEConfig {
        let mut default_fide_config: FIDEConfig =
        ...
        default_fide_config.color_theme = DEFAULT_DARK_THEME_COLOR_CONFIG.clone();
        default_fide_config.icon_theme = DEFAULT_ICON_THEME_ICON_CONFIG.clone();
        default_fide_config.resolve_colors(None);
        default_fide_config
    }

    fn resolve_theme(&mut self, workspace: &FIDEWorkspace) {
        let default_fide_config = DEFAULT_LAPCE_CONFIG.clone();
        ...
        self.resolve_colors(Some(&default_fide_config));
        self.update_id();
    }
    ...
    pub fn set_color_theme(&mut self, workspace: &FIDEWorkspace, theme: &str) {
    ...
    /// Note that this does not save the config.
    pub fn set_icon_theme(&mut self, workspace: &FIDEWorkspace, theme: &str) {
        self.core.icon_theme = theme.to_string();
        self.resolve_theme(workspace);
    }

    pub fn set_modal(&mut self, _workspace: &FIDEWorkspace, modal: bool) {
    ...
    fn resolve_colors(&mut self, default_config: Option<&FIDEConfig>) {
        ...
        let fg = self.color(FIDEColor::EDITOR_FOREGROUND).to_rgba8();
        let bg = self.color(FIDEColor::EDITOR_BACKGROUND).to_rgba8();
    ...
    pub fn files_svg(&self, paths: &[&Path]) -> (String, Option<Color>) {
        ...
        if let Some(svg) = svg {
            let color = if self.icon_theme.use_editor_color.unwrap_or(false) {
                Some(self.color(FIDEColor::LAPCE_ICON_ACTIVE))
            } else {
                None
            };
            (svg, color)
    ...
    pub fn symbol_svg(&self, kind: &SymbolKind) -> Option<String> {
        let kind_str = match *kind {
            SymbolKind::ARRAY => FIDEIcons::SYMBOL_KIND_ARRAY,
            SymbolKind::BOOLEAN => FIDEIcons::SYMBOL_KIND_BOOLEAN,
            SymbolKind::CLASS => FIDEIcons::SYMBOL_KIND_CLASS,
            SymbolKind::CONSTANT => FIDEIcons::SYMBOL_KIND_CONSTANT,
            SymbolKind::ENUM_MEMBER => FIDEIcons::SYMBOL_KIND_ENUM_MEMBER,
            SymbolKind::ENUM => FIDEIcons::SYMBOL_KIND_ENUM,
            SymbolKind::EVENT => FIDEIcons::SYMBOL_KIND_EVENT,
            SymbolKind::FIELD => FIDEIcons::SYMBOL_KIND_FIELD,
            SymbolKind::FILE => FIDEIcons::SYMBOL_KIND_FILE,
            SymbolKind::INTERFACE => FIDEIcons::SYMBOL_KIND_INTERFACE,
            SymbolKind::KEY => FIDEIcons::SYMBOL_KIND_KEY,
            SymbolKind::FUNCTION => FIDEIcons::SYMBOL_KIND_FUNCTION,
            SymbolKind::METHOD => FIDEIcons::SYMBOL_KIND_METHOD,
            SymbolKind::OBJECT => FIDEIcons::SYMBOL_KIND_OBJECT,
            SymbolKind::NAMESPACE => FIDEIcons::SYMBOL_KIND_NAMESPACE,
            SymbolKind::NUMBER => FIDEIcons::SYMBOL_KIND_NUMBER,
            SymbolKind::OPERATOR => FIDEIcons::SYMBOL_KIND_OPERATOR,
            SymbolKind::TYPE_PARAMETER => FIDEIcons::SYMBOL_KIND_TYPE_PARAMETER,
            SymbolKind::PROPERTY => FIDEIcons::SYMBOL_KIND_PROPERTY,
            SymbolKind::STRING => FIDEIcons::SYMBOL_KIND_STRING,
            SymbolKind::STRUCT => FIDEIcons::SYMBOL_KIND_STRUCT,
            SymbolKind::VARIABLE => FIDEIcons::SYMBOL_KIND_VARIABLE,
            _ => return None,
        };
    ...
    fn terminal_get_named_color(
        ...
        let (color, alpha) = match color {
            alacritty_terminal::vte::ansi::NamedColor::Cursor => {
                (FIDEColor::TERMINAL_CURSOR, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Foreground => {
                (FIDEColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Background => {
                (FIDEColor::TERMINAL_BACKGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Blue => {
                (FIDEColor::TERMINAL_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Green => {
                (FIDEColor::TERMINAL_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Yellow => {
                (FIDEColor::TERMINAL_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Red => {
                (FIDEColor::TERMINAL_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::White => {
                (FIDEColor::TERMINAL_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Black => {
                (FIDEColor::TERMINAL_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Cyan => {
                (FIDEColor::TERMINAL_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Magenta => {
                (FIDEColor::TERMINAL_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlue => {
                (FIDEColor::TERMINAL_BRIGHT_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightGreen => {
                (FIDEColor::TERMINAL_BRIGHT_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightYellow => {
                (FIDEColor::TERMINAL_BRIGHT_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightRed => {
                (FIDEColor::TERMINAL_BRIGHT_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightWhite => {
                (FIDEColor::TERMINAL_BRIGHT_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlack => {
                (FIDEColor::TERMINAL_BRIGHT_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightCyan => {
                (FIDEColor::TERMINAL_BRIGHT_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightMagenta => {
                (FIDEColor::TERMINAL_BRIGHT_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightForeground => {
                (FIDEColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlack => {
                (FIDEColor::TERMINAL_BLACK, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimRed => {
                (FIDEColor::TERMINAL_RED, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimGreen => {
                (FIDEColor::TERMINAL_GREEN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimYellow => {
                (FIDEColor::TERMINAL_YELLOW, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlue => {
                (FIDEColor::TERMINAL_BLUE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimMagenta => {
                (FIDEColor::TERMINAL_MAGENTA, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimCyan => {
                (FIDEColor::TERMINAL_CYAN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimWhite => {
                (FIDEColor::TERMINAL_WHITE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimForeground => {
                (FIDEColor::TERMINAL_FOREGROUND, 0.66)
            }
        };
