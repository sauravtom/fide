...
use crate::{
    ...
    command::{
        CommandKind, InternalCommand, FIDECommand, FIDEWorkbenchCommand,
        WindowCommand,
    },
    config::{
        FIDEConfig, color::FIDEColor, icon::FIDEIcons, ui::TabSeparatorHeight,
    ...
    workspace::{FIDEWorkspace, FIDEWorkspaceType},
};
...
#[derive(Debug)]
struct Cli {
    /// Launch new window even if FIDE is already running
    ...
    #[clap(value_parser = fide_proxy::cli::parse_file_line_column)]
...
pub struct AppData {
    ...
    pub config: RwSignal<Arc<FIDEConfig>>,
...
impl AppData {
    pub fn reload_config(&self) {
        let config =
            FIDEConfig::load(&FIDEWorkspace::default(), &[], &self.plugin_paths);
    ...
    pub fn new_window(&self, folder: Option<PathBuf>) {
        let config = self
            ...
            .or_else(|| {
                let db: Arc<FIDEDb> = use_context().unwrap();
    ...
    fn create_windows(
        ...
        if !dirs.is_empty() {
            ...
            for dir in dirs {
                ...
                {
                    FIDEWorkspaceType::RemoteWSL(crate::workspace::WslHost {
                        host: String::new(),
                    })
                } else {
                    FIDEWorkspaceType::Local
                };
                #[cfg(not(windows))]
                let workspace_type = FIDEWorkspaceType::Local;
        ...
        if inital_windows == 0 {
            let mut info = db.get_window().unwrap_or_else(|_| WindowInfo {
                ...
                tabs: TabsInfo {
                    active_tab: 0,
                    workspaces: vec![FIDEWorkspace::default()],
                },
            });
            info.tabs = TabsInfo {
                active_tab: 0,
    ...
    let view_fn = move |(i, layout_rect, child): (
        ...
        let child_view = {
                            ...
                            .apply_if(
                                !info.with(|info| info.is_pristine)
                                    && config.ui.tab_close_button
                                        == TabCloseButton::Off,
                                |s| s.color(config.color(FIDEColor::LAPCE_WARN)),
                            )
                    })
            })
            .style(|s| s.padding(4.));
            ...
            .style(move |s| {
                s.items_center()
                    ...
                    .border_right(1.0)
                    .border_color(config.get().color(FIDEColor::LAPCE_BORDER))
                    .padding_horiz(6.)
                    .gap(6.)
                    .grid()
            ...
            child_view
                ...
                .dragging_style(move |s| {
                    ...
                    s.border(1.0)
                        ...
                        .background(
                            config
                                .color(FIDEColor::PANEL_BACKGROUND)
                                .multiply_alpha(0.7),
                        )
                        .border_color(config.color(FIDEColor::LAPCE_BORDER))
                })
                ...
                .style(move |s| {
                    s.size_full()
                        ...
                        .border_color(config.get().color(if is_focused() {
                            FIDEColor::LAPCE_TAB_ACTIVE_UNDERLINE
                        } else {
                            FIDEColor::LAPCE_TAB_INACTIVE_UNDERLINE
                        }))
                })
                .style(|s| {
                    s.absolute()
                ...
                .style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .border_color(
                            config
                                .get()
                                .color(FIDEColor::LAPCE_TAB_ACTIVE_UNDERLINE)
                                .multiply_alpha(0.5),
                        )
                })
        ...
        .style(move |s| {
            ...
            s.height_full()
                ...
                .hover(|s| s.background(config.color(FIDEColor::HOVER_BACKGROUND)))
        })
            ...
            (
                clip(empty().style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .background(config.color(FIDEColor::PANEL_BACKGROUND))
                        .box_shadow_blur(3.0)
                        .box_shadow_color(
                            config.color(FIDEColor::LAPCE_DROPDOWN_SHADOW),
                        )
                }))
                .style(move |s| {
                ...
                stack((
                    clickable_icon(
                        || FIDEIcons::TAB_PREVIOUS,
                        move || {
                            workbench_command
                                .send(FIDEWorkbenchCommand::PreviousEditorTab);
                        },
                        || false,
                        || false,
                        || "Previous Tab",
                    ...
                    clickable_icon(
                        ...
                        move || {
                            workbench_command
                                .send(FIDEWorkbenchCommand::NextEditorTab);
                        },
                        || false,
                        || false,
                        || "Next Tab",
                    ...
                    stack((
                        ...
                        .style(move |s| {
                            s.height_full()
                                ...
                                .border_right(1.0)
                                .border_color(
                                    config.get().color(FIDEColor::LAPCE_BORDER),
                                )
                        }),
                        container(
                            editor_container_view(
                ...
                .style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .background(
                            config
                                .get()
                                .color(FIDEColor::EDITOR_DRAG_DROP_BACKGROUND),
                        )
                })
...
fn split_resize_border(
    ...
    config: ReadSignal<Arc<FIDEConfig>>,
) -> impl View {
            ...
            .style(move |s| {
                ...
                s.position(Position::Absolute)
                    ...
                    .apply_if(is_dragging, |s| {
                        ...
                        .background(config.get().color(FIDEColor::EDITOR_CARET))
                    })
                    .hover(|s| {
                        s.cursor(match direction {
                            SplitDirection::Vertical => CursorStyle::ColResize,
    ...
    dyn_stack(
        ...
        move |(_, content)| {
            container(empty().style(move |s| {
                ...
                .background(config.get().color(FIDEColor::LAPCE_BORDER))
            }))
            .style(move |s| {
...
fn main_split(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.border_color(config.color(FIDEColor::LAPCE_BORDER))
            .background(config.color(FIDEColor::EDITOR_BACKGROUND))
    ...
    let view = container(
        svg(move || config.get().ui_svg(icon()))
            .style(move |s| {
                ...
                let size = config.ui.icon_size() as f32;
                s.size(size, size)
                    .color(config.color(FIDEColor::LAPCE_ICON_ACTIVE))
                    .disabled(|s| {
                        s.color(config.color(FIDEColor::LAPCE_ICON_INACTIVE))
    ...
    .style(move |s| {
        ...
        s.padding(4.0)
            ...
            .apply_if(active_fn(), |s| {
                s.border_color(config.color(FIDEColor::EDITOR_CARET))
            })
            .hover(|s| {
                s.cursor(CursorStyle::Pointer)
                    .background(config.color(FIDEColor::PANEL_HOVERED_BACKGROUND))
            })
            .active(|s| {
                s.background(
                    config.color(FIDEColor::PANEL_HOVERED_ACTIVE_BACKGROUND),
                )
            })
    });
...
fn tooltip_tip<V: View + 'static>(
    ...
    container(child).style(move |s| {
        ...
        s.padding_horiz(10.0)
            ...
            .color(config.color(FIDEColor::TOOLTIP_FOREGROUND))
            .background(config.color(FIDEColor::TOOLTIP_BACKGROUND))
            .border(1)
            .border_radius(6)
            .border_color(config.color(FIDEColor::LAPCE_BORDER))
            .box_shadow_blur(3.0)
            .box_shadow_color(config.color(FIDEColor::LAPCE_DROPDOWN_SHADOW))
            .margin_left(0.0)
            .margin_top(4.0)
    })
}
            ...
            container(
                stack((
                    ...
                    focus_text(
                        ...
                        move || config.get().color(FIDEColor::EDITOR_FOCUS),
                    )
                    ...
                    .style(move |s| {
                        s.color(config.get().color(FIDEColor::EDITOR_DIM))
                            .min_width(0.0)
                            .flex_grow(1.0)
                            .flex_basis(0.0)
                    }),
            ...
            container(
                stack((
                    svg(move || {
                        ...
                        config
                            .symbol_svg(&kind)
                            .unwrap_or_else(|| config.ui_svg(FIDEIcons::FILE))
                    })
                    .style(move |s| {
                        let config = config.get();
                        let size = config.ui.icon_size() as f32;
                        s.min_width(size).size(size, size).margin_right(5.0).color(
                            config.symbol_color(&kind).unwrap_or_else(|| {
                                config.color(FIDEColor::LAPCE_ICON_ACTIVE)
                            }),
                        )
                    }),
                    focus_text(
            ...
            container(
                stack((
                    svg(move || {
                        let config = config.get();
                        match mode {
                            RunDebugMode::Run => config.ui_svg(FIDEIcons::START),
                            RunDebugMode::Debug => config.ui_svg(FIDEIcons::DEBUG),
                        }
                    })
                    .style(move |s| {
                        let config = config.get();
    ...
    .style(move |s| {
        s.width_full()
            ...
            .apply_if(index.get() == i, |style| {
                style.background(
                    config.get().color(FIDEColor::PALETTE_CURRENT_BACKGROUND),
                )
            })
    })
}
                    ...
                    .style(move |s| {
                        s.width_full().cursor(CursorStyle::Pointer).hover(|s| {
                            s.background(
                                config
                                    .get()
                                    .color(FIDEColor::PANEL_HOVERED_BACKGROUND),
                            )
                        })
                    })
                },
    ...
    container(
        ...
        .style(move |s| {
            ...
            s.width(config.ui.palette_width() as f64)
                ...
                .border_color(config.color(FIDEColor::LAPCE_BORDER))
                .flex_col()
                .background(config.color(FIDEColor::PALETTE_BACKGROUND))
                .pointer_events_auto()
        }),
    )
    .style(move |s| {
            ...
            stack((
                svg(move || {
                    if let MessageType::ERROR = message.typ {
                        config.get().ui_svg(FIDEIcons::ERROR)
                    } else {
                        config.get().ui_svg(FIDEIcons::WARNING)
                    }
                })
                .style(move |s| {
                    let config = config.get();
                    ...
                    let color = if let MessageType::ERROR = message.typ {
                        config.color(FIDEColor::LAPCE_ERROR)
                    } else {
                        config.color(FIDEColor::LAPCE_WARN)
                    };
                    s.min_width(size)
                        .size(size, size)
                        .margin_right(10.0)
...
fn completion(window_tab_data: Rc<WindowTabData>) -> impl View {
                    ...
                    .style(move |s| {
                        ...
                        s.padding_horiz(5.0)
                            ...
                            .apply_if(active.get() == i, |s| {
                                s.background(
                                    config.color(FIDEColor::COMPLETION_CURRENT),
                                )
                            })
                            .hover(move |s| {
                                s.background(
    ...
    .style(move |s| {
        ...
        s.position(Position::Absolute)
            ...
            .margin_left(origin.x as f32)
            .margin_top(origin.y as f32)
            .background(config.color(FIDEColor::COMPLETION_BACKGROUND))
...
fn code_action(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        .margin_left(origin.x as f32)
        .margin_top(origin.y as f32)
        .background(config.get().color(FIDEColor::COMPLETION_BACKGROUND))
        .border_radius(6.0)
    })
...
fn rename(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.position(Position::Absolute)
            ...
            .margin_left(origin.x as f32)
            .margin_top(origin.y as f32)
            .background(config.get().color(FIDEColor::PANEL_BACKGROUND))
            .border_radius(6.0)
            .padding(6.0)
    })
...
fn window_tab(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        let config = config.get();
        s.size_full()
            .color(config.color(FIDEColor::EDITOR_FOREGROUND))
            .background(config.color(FIDEColor::EDITOR_BACKGROUND))
            .font_size(config.ui.font_size() as f32)
            ...
            .class(floem::views::scroll::Handle, |s| {
                s.background(config.color(FIDEColor::LAPCE_SCROLL_BAR))
            })
    })
...
fn workspace_title(workspace: &FIDEWorkspace) -> Option<String> {
    let p = workspace.path.as_ref()?;
    let dir = p.file_name().unwrap_or(p.as_os_str()).to_string_lossy();
    Some(match &workspace.kind {
        FIDEWorkspaceType::Local => format!("{dir}"),
        FIDEWorkspaceType::RemoteSSH(remote) => format!("{dir} [{remote}]"),
        #[cfg(windows)]
        FIDEWorkspaceType::RemoteWSL(remote) => format!("{dir} [{remote}]"),
    })
}
        ...
        stack((
            ...
            .dragging_style(move |s| {
                ...
                s.border(1.0)
                    ...
                    .color(
                        config
                            .color(FIDEColor::EDITOR_FOREGROUND)
                            .multiply_alpha(0.7),
                    )
                    .background(
                        config
            ...
            empty().style(move |s| {
                ...
                s.absolute()
                    ...
                    .height_full()
                    .border_color(
                        config.get().color(FIDEColor::LAPCE_TAB_ACTIVE_UNDERLINE),
                    )
        ...
        container(clickable_icon(
            ...
            move || {
                window_data.run_window_command(WindowCommand::NewWorkspaceTab {
                    workspace: FIDEWorkspace::default(),
                    end: true,
                });
            },
            || false,
...
fn window(window_data: WindowData) -> impl View {
    ...
    .window_title(move || {
        ...
        match workspace {
            Some(workspace) => format!("{workspace} - FIDE"),
            None => "FIDE".to_string(),
        }
    })
    ...
    .window_menu(move || {
        ...
        if let Some((_, window_tab)) = window_tab {
            ...
            let fide_command = window_tab.common.fide_command;
            window_menu(fide_command, workbench_command)
        } else {
            Menu::new("FIDE")
        }
    })
...
pub fn launch() {
    ...
    trace!(TraceLevel::INFO, "Starting up FIDE..");
    ...
    if !cli.wait {
        ...
        if let Err(why) = cmd
            ...
            .stderr(stderr)
            .stdout(stdout)
            .env("LAPCE_LOG", "fide_app::app=error,off")
            .spawn()
        {
            eprintln!("Failed to launch fide: {why}");
            std::process::exit(1);
        };
        return;
    }
    ...
    if let Err(err) = fide_proxy::register_fide_path() {
        tracing::error!("{:?}", err);
    }
    let db = match FIDEDb::new() {
        Ok(db) => Arc::new(db),
        Err(e) => {
            #[cfg(windows)]
            logging::error_modal("Error", &format!("Failed to create FIDEDb: {e}"));

            trace!(TraceLevel::ERROR, "Failed to create FIDEDb: {e}");
            std::process::exit(1);
        }
    };
    let scope = Scope::new();
    ...
    if let Some(path) = FIDEConfig::settings_file() {
    ...
    if let Some(path) = FIDEConfig::keymaps_file() {
        if let Err(err) = watcher.watch(&path, notify::RecursiveMode::Recursive) {
            tracing::error!("{:?}", err);
        }
    }
    ...
    let config = FIDEConfig::load(&FIDEWorkspace::default(), &[], &plugin_paths);
...
pub fn try_open_in_existing_process(
    ...
    fide_rpc::stdio::write_msg(&mut socket, msg)?;
        ...
        std::thread::spawn(move || -> Result<()> {
            ...
            loop {
                let msg: Option<CoreMessage> =
                    fide_rpc::stdio::read_msg(&mut reader)?;
...
pub fn window_menu(
    fide_command: Listener<FIDECommand>,
    workbench_command: Listener<FIDEWorkbenchCommand>,
) -> Menu {
    Menu::new("FIDE")
        .entry({
            let mut menu = Menu::new("FIDE")
                .entry(MenuItem::new("About FIDE").action(move || {
                    workbench_command.send(FIDEWorkbenchCommand::ShowAbout)
                }))
                .separator()
                .entry(
                    Menu::new("Settings...")
                        .entry(MenuItem::new("Open Settings").action(move || {
                            workbench_command
                                .send(FIDEWorkbenchCommand::OpenSettings);
                        }))
                        .entry(MenuItem::new("Open Keyboard Shortcuts").action(
                            move || {
                                workbench_command.send(
                                    FIDEWorkbenchCommand::OpenKeyboardShortcuts,
                                );
                            },
                        )),
                )
                .separator()
                .entry(MenuItem::new("Quit FIDE").action(move || {
                    workbench_command.send(FIDEWorkbenchCommand::Quit);
                }));
            if cfg!(target_os = "macos") {
                menu = menu
                    .separator()
                    .entry(MenuItem::new("Hide FIDE"))
        ...
        .entry(
            Menu::new("File")
                .entry(MenuItem::new("New File").action(move || {
                    workbench_command.send(FIDEWorkbenchCommand::NewFile);
                }))
                .separator()
                .entry(MenuItem::new("Open").action(move || {
                    workbench_command.send(FIDEWorkbenchCommand::OpenFile);
                }))
                .entry(MenuItem::new("Open Folder").action(move || {
                    workbench_command.send(FIDEWorkbenchCommand::OpenFolder);
                }))
                .separator()
                .entry(MenuItem::new("Save").action(move || {
                    fide_command.send(FIDECommand {
                        kind: CommandKind::Focus(FocusCommand::Save),
                        data: None,
                    });
                }))
                .entry(MenuItem::new("Save All").action(move || {
                    workbench_command.send(FIDEWorkbenchCommand::SaveAll);
                }))
                .separator()
                .entry(MenuItem::new("Close Folder").action(move || {
                    workbench_command.send(FIDEWorkbenchCommand::CloseFolder);
                }))
                .entry(MenuItem::new("Close Window").action(move || {
                    workbench_command.send(FIDEWorkbenchCommand::CloseWindow);
                })),
        )
        .entry(
            Menu::new("Edit")
