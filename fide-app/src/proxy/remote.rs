...
pub fn start_remote(
    ...
    let remote_proxy_path = match platform {
        Windows => format!(
            "%HOMEDRIVE%%HOMEPATH%\\AppData\\Local\\fide\\{}\\data\\proxy",
            meta::NAME
        ),
        Darwin => format!(
            "~/Library/Application\\ Support/dev.fide.{}/proxy",
            meta::NAME
        ),
        _ => {
    ...
    let remote_proxy_file = match platform {
        Windows => format!("{remote_proxy_path}\\fide.exe"),
        _ => format!("{remote_proxy_path}/fide"),
    };
    ...
    if !remote
        ...
        .map(|output| {
            if meta::RELEASE == ReleaseType::Debug {
                String::from_utf8_lossy(&output.stdout).starts_with("FIDE-proxy")
            } else {
                String::from_utf8_lossy(&output.stdout).trim()
                    == format!("FIDE-proxy {}", meta::VERSION)
            }
        })
        .unwrap_or(false)
    {
...
fn download_remote(
    ...
    let script_install = match platform {
        HostPlatform::Windows => {
            ...
            let remote_proxy_script = "${env:TEMP}\\fide-proxy.ps1";
    ...
    if !script_install.success() {
        ...
        if !cmd.success() {
            let proxy_filename = format!("fide-proxy-{platform}-{architecture}");
            ...
            let url = format!(
                "https://github.com/fide/fide/releases/download/{proxy_version}/{proxy_filename}.gz"
            );
            debug!("proxy download URI: {url}");
            let mut resp = fide_proxy::get_url(url, None).expect("request failed");
