...
pub fn problem_panel(
    ...
    PanelBuilder::new(config, position)
        .add_style(
            ...
            move |s| {
                s.border_color(config.get().color(FIDEColor::LAPCE_BORDER))
...
fn file_view(
    ...
    let icon = match severity {
        DiagnosticSeverity::ERROR => FIDEIcons::ERROR,
        _ => FIDEIcons::WARNING,
    };
    let icon_color = move || {
        let config = config.get();
        match severity {
            DiagnosticSeverity::ERROR => config.color(FIDEColor::LAPCE_ERROR),
            _ => config.color(FIDEColor::LAPCE_WARN),
        }
    };
...
fn related_view(
    ...
    stack((
        ...
        stack((
            svg(move || config.get().ui_svg(FIDEIcons::LINK)).style(move |s| {
                let config = config.get();
                let size = config.ui.icon_size() as f32;
                s.size(size, size)
