...
use fide_core::buffer::rope_text::RopeText;
use fide_rpc::source_control::FileDiff;
...
use crate::{
    ...
    config::{color::FIDEColor, icon::FIDEIcons},
                    ...
                    let view = stack((
                        ...
                        label(|| "Commit Message".to_string()).style(move |s| {
                            ...
                            s.absolute()
                                ...
                                .color(config.color(FIDEColor::EDITOR_DIM))
                        ...
                        s.margin_top(10.0)
                            ...
                            .active(|s| {
                                s.background(config.color(
                                    FIDEColor::PANEL_HOVERED_ACTIVE_BACKGROUND,
                                ))
                            })
                            .selectable(false)
                    })
...
fn file_diffs_view(source_control: SourceControlData) -> impl View {
    ...
    let fide_command = source_control.common.fide_command;
    ...
    let view_fn = move |(path, (diff, checked)): (PathBuf, (FileDiff, bool))| {
        ...
        stack((
            ...
            label(move || folder.clone()).style(move |s| {
                s.text_ellipsis()
                    .flex_grow(1.0)
                    .flex_basis(0.0)
                    .color(config.get().color(FIDEColor::EDITOR_DIM))
                    .min_width(0.0)
                    .selectable(false)
            }),
            container({
                svg(move || {
                    let svg = match &diff {
                        FileDiff::Modified(_) => FIDEIcons::SCM_DIFF_MODIFIED,
                        FileDiff::Added(_) => FIDEIcons::SCM_DIFF_ADDED,
                        FileDiff::Deleted(_) => FIDEIcons::SCM_DIFF_REMOVED,
                        FileDiff::Renamed(_, _) => FIDEIcons::SCM_DIFF_RENAMED,
                    };
                ...
                .style(move |s| {
                    ...
                    let color = match &diff_for_style {
                        FileDiff::Modified(_) => FIDEColor::SOURCE_CONTROL_MODIFIED,
                        FileDiff::Added(_) => FIDEColor::SOURCE_CONTROL_ADDED,
                        FileDiff::Deleted(_) => FIDEColor::SOURCE_CONTROL_REMOVED,
                        FileDiff::Renamed(_, _) => {
                            FIDEColor::SOURCE_CONTROL_MODIFIED
                        }
                    };
            ...
            let discard = move || {
                fide_command.send(FIDECommand {
                    kind: CommandKind::Workbench(
                        FIDEWorkbenchCommand::SourceControlDiscardTargetFileChanges,
                    ),
        ...
        .style(move |s| {
            ...
            s.padding_left(10.0)
                ...
                .hover(|s| {
                    s.background(config.color(FIDEColor::PANEL_HOVERED_BACKGROUND))
                })
        })
    };
