...
use fide_core::{language::FIDELanguage, syntax::Syntax};
use lapce_xi_rope::Rope;
use lsp_types::MarkedString;
use pulldown_cmark::{CodeBlockKind, CowStr, Event, Options, Parser, Tag};
use smallvec::SmallVec;

use crate::config::{FIDEConfig, color::FIDEColor};
                        ...
                        Tag::CodeBlock(kind) => {
                            let language =
                                if let CodeBlockKind::Fenced(language) = kind {
                                    md_language_to_fide_language(language)
                                } else {
                                    None
                                };
            ...
            Event::Html(text) => {
                attr_list.add_span(
                    ...
                    default_attrs
                        ...
                        .color(config.color(FIDEColor::MARKDOWN_BLOCKQUOTE)),
                );
                current_text.push_str(&text);
                pos += text.len();
                builder_dirty = true;
...
fn attribute_for_tag<'a>(
    ...
    match tag {
        ...
        Tag::Link {
            ...
            // TODO: Link support
            Some(default_attrs.color(config.color(FIDEColor::EDITOR_LINK)))
        }
...
fn md_language_to_fide_language(lang: &str) -> Option<FIDELanguage> {
    // TODO: There are many other names commonly used that should be supported
    FIDELanguage::from_name(lang)
}
