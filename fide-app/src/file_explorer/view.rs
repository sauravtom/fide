...
use fide_core::selection::Selection;
use fide_rpc::{
...
fn file_node_text_color(
    ...
    let color = match diff {
        ...
        Some(FileDiffKind::Added) => FIDEColor::SOURCE_CONTROL_ADDED,
        Some(FileDiffKind::Deleted) => FIDEColor::SOURCE_CONTROL_REMOVED,
        None => FIDEColor::PANEL_FOREGROUND,
    };
                        ...
                        move |s| {
                            s.height(ui_line_height.get())
                                .color(
                                    config
                                        .get()
                                        .color(FIDEColor::PANEL_FOREGROUND_DIM),
                                )
                                .selectable(false)
                        },
                    ),
            ...
            stack((
                ...
                label(move || err.clone()).style(move |s| {
                    ...
                    let error_background_color =
                        config.color(FIDEColor::ERROR_LENS_ERROR_BACKGROUND);
                    ...
                    s.position(Position::Absolute)
                        ...
                        .width_full()
                        .color(config.color(FIDEColor::ERROR_LENS_ERROR_FOREGROUND))
                ...
                let view = stack((
                    ...
                    }),
                    {
                        ...
                        svg(move || {
                            ...
                            if is_dir {
                                let svg_str = match open {
                                    true => FIDEIcons::DIRECTORY_OPENED,
                                    false => FIDEIcons::DIRECTORY_CLOSED,
                                };
                                config.ui_svg(svg_str)
                            } else if let Some(path) = kind.path() {
                                config.file_svg(path).0
                            } else {
                                config.ui_svg(FIDEIcons::FILE)
                            }
                        })
                        .style(move |s| {
                            let config = config.get();
                        ...
                        s.padding_right(15.0)
                            ...
                            .apply_if(
                                ...
                                |x| {
                                    x.background(
                                        config.get().color(
                                            FIDEColor::PANEL_CURRENT_BACKGROUND,
                                        ),
                                    )
                                },
                            )
