...
use fide_core::{directory::Directory, meta};
...
pub fn get_latest_release() -> Result<ReleaseInfo> {
    let url = match meta::RELEASE {
        ...
        meta::ReleaseType::Nightly => {
            "https://api.github.com/repos/fide/fide/releases/tags/nightly"
        }
        _ => "https://api.github.com/repos/fide/fide/releases/latest",
    };

    let resp = fide_proxy::get_url(url, Some("FIDE"))?;
...
pub fn download_release(release: &ReleaseInfo) -> Result<PathBuf> {
    ...
    let name = match std::env::consts::OS {
        "macos" => "FIDE-macos.dmg",
        "linux" => match std::env::consts::ARCH {
            "aarch64" => "fide-linux-arm64.tar.gz",
            "x86_64" => "fide-linux-amd64.tar.gz",
            _ => return Err(anyhow!("arch not supported")),
        },
        #[cfg(feature = "portable")]
        "windows" => "FIDE-windows-portable.zip",
        #[cfg(not(feature = "portable"))]
        "windows" => "FIDE-windows.msi",
    ...
    for asset in &release.assets {
        if asset.name == name {
            let mut resp = fide_proxy::get_url(&asset.browser_download_url, None)?;
            if !resp.status().is_success() {
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::remove_dir_all(dest.join("FIDE.app"))?;
    fs_extra::copy_items(
        &[info.mount_point.join("FIDE.app")],
        dest,
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::copy(parent.join("FIDE").join("fide"), process_path)?;
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::rename(process_path, dst_parent.join("fide.exe.bak"))?;
    std::fs::copy(parent.join("fide.exe"), process_path)?;
...
pub fn restart(path: &Path) -> Result<()> {
    ...
    let fide_exe = std::env::current_exe()
        .map_err(|err| anyhow!("can't get path to exe").context(err))?;
    let fide_exe = fide_exe
        .to_str()
        .ok_or_else(|| anyhow!("can't convert exe path to str"))?;

    std::process::Command::new("cmd")
        .raw_arg(format!(
            r#"/C taskkill /PID {process_id} & msiexec /i "{path}" /qb & start "" "{fide_exe}""#,
        ))
        .creation_flags(DETACHED_PROCESS)
        .spawn()?;
    Ok(())
...
pub fn cleanup() {
    ...
    if let Ok(process_path) = std::env::current_exe() {
        if let Some(dst_parent) = process_path.parent() {
            if let Err(err) = std::fs::remove_file(dst_parent.join("fide.exe.bak"))
            {
                tracing::error!("{:?}", err);
            }
        }
