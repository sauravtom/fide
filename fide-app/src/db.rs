...
use crate::{
    ...
    workspace::{FIDEWorkspace, WorkspaceInfo},
};
...
pub enum SaveEvent {
    App(AppInfo),
    Workspace(FIDEWorkspace, WorkspaceInfo),
    RecentWorkspace(FIDEWorkspace),
    Doc(DocInfo),
    DisabledVolts(Vec<VoltID>),
    WorkspaceDisabledVolts(Arc<FIDEWorkspace>, Vec<VoltID>),
...
impl FIDEDb {
    ...
    pub fn recent_workspaces(&self) -> Result<Vec<FIDEWorkspace>> {
        let workspaces =
            std::fs::read_to_string(self.folder.join(RECENT_WORKSPACES))?;
        let workspaces: Vec<FIDEWorkspace> = serde_json::from_str(&workspaces)?;
        Ok(workspaces)
    }

    pub fn update_recent_workspace(&self, workspace: &FIDEWorkspace) -> Result<()> {
    ...
    fn insert_recent_workspace(&self, workspace: FIDEWorkspace) -> Result<()> {
...
fn workspace_folder_name(workspace: &FIDEWorkspace) -> String {
