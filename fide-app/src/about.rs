...
use fide_core::{command::FocusCommand, meta::VERSION, mode::Mode};

use crate::{
    command::{CommandExecuted, CommandKind},
    config::color::FIDEColor,
...
impl AboutUri {
    const LAPCE: &'static str = "https://fide.dev";
    const GITHUB: &'static str = "https://github.com/fide/fide";
    const MATRIX: &'static str = "https://matrix.to/#/#fide-editor:matrix.org";
        ...
        stack((
            svg(move || (config.get()).logo_svg()).style(move |s| {
                s.size(logo_size, logo_size)
                    .color(config.get().color(FIDEColor::EDITOR_FOREGROUND))
            }),
            label(|| "FIDE".to_string()).style(move |s| {
                s.font_bold()
                    .margin_top(10.0)
                    .color(config.get().color(FIDEColor::EDITOR_FOREGROUND))
            }),
            ...
            web_link(
                ...
                move || config.get().color(FIDEColor::EDITOR_LINK),
                internal_command,
            )
...
fn exclusive_popup<V: View + 'static>(
    ...
    .style(move |s| {
        ...
        .background(
            config
                .get()
                .color(FIDEColor::LAPCE_DROPDOWN_SHADOW)
                .multiply_alpha(0.5),
        )
    })
}
