        ...
        let injection_callback = |language: &InjectionLanguageMarker| {
            let language = match language {
                InjectionLanguageMarker::Name(name) => {
                    FIDELanguage::from_name(name)
                }
                InjectionLanguageMarker::Filename(path) => {
                    FIDELanguage::from_path_raw(path)
                }
                InjectionLanguageMarker::Shebang(id) => FIDELanguage::from_name(id),
            };
            language
...
impl Syntax {
    pub fn init(path: &Path) -> Syntax {
        let language = FIDELanguage::from_path(path);
        Syntax::from_language(language)
    }

    pub fn plaintext() -> Syntax {
        Self::from_language(FIDELanguage::PlainText)
    }

    pub fn from_language(language: FIDELanguage) -> Syntax {
    ...
    pub fn find_enclosing_pair(&self, offset: usize) -> Option<(usize, usize)> {
        if self.language == FIDELanguage::Markdown {
