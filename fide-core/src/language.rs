...
use fide_rpc::style::{LineStyle, Style};
...
/// indices into the LANGUAGES array.  See method `FIDELanguage::properties`.
///
...
/// `FIDELanguage` as they will be accessed using the enum variants as indices.
const LANGUAGES: &[SyntaxProperties] = &[
    // Undetected/unmatched fallback or just plain file
    SyntaxProperties {
        id: FIDELanguage::PlainText,
        indent: Indent::tab(),
        files: &[],
        extensions: &["txt"],
    ...
    SyntaxProperties {
        id: FIDELanguage::KotlinBuildScript,
        indent: Indent::space(2),
        files: &[],
        extensions: &["kts"],
        comment: CommentProperties {
    ...
    SyntaxProperties {
        id: FIDELanguage::MarkdownInline,
        indent: Indent::space(4),
    ...
    SyntaxProperties {
        id: FIDELanguage::OcamlInterface,
        indent: Indent::space(2),
        files: &[],
        extensions: &["mli"],
        comment: comment_properties!("(*"),
    ...
    SyntaxProperties {
        id: FIDELanguage::SshClientConfig,
        indent: Indent::tab(),
        files: &[],
        extensions: &[],
        comment: comment_properties!(),
...
impl FIDELanguage {
    ...
    pub fn from_path(path: &Path) -> FIDELanguage {
        Self::from_path_raw(path).unwrap_or(FIDELanguage::PlainText)
    }

    pub fn from_path_raw(path: &Path) -> Option<FIDELanguage> {
    ...
    pub fn from_name(name: &str) -> Option<FIDELanguage> {
        match FIDELanguage::from_str(name.to_lowercase().as_str()) {
            Ok(v) => Some(v),
            Err(e) => {
                event!(Level::DEBUG, "failed parsing `{name}` FIDELanguage: {e}");
                None
            }
        }
    }
    ...
    fn get_grammar(&self) -> Option<tree_sitter::Language> {
        ...
        if let Some(grammars_dir) = Directory::grammars_directory() {
            match self::load_grammar(&grammar_name, &grammar_fn_name, &grammars_dir)
            {
                ...
                Err(err) => {
                    if self != &FIDELanguage::PlainText {
...
mod tests {
    ...
    use super::FIDELanguage;

    #[test]
    fn test_lanaguage_from_path() {
        let l = FIDELanguage::from_path(&PathBuf::new().join("test.rs"));
        assert_eq!(l, FIDELanguage::Rust);
    }
}
